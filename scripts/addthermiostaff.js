#!/usr/bin/env node
// scripts/addthermiostaff.js
// Creates or replaces the Portal Admin (superadmin) account.
// Usage: node scripts/addthermiostaff.js  OR  npm run addthermiostaff
'use strict';
...
function validatePassword(pw) {
  ...
  if ((pw.match(/[0-9]/g) || []).length < 2)         errors.push('At least 2 digits (0–9)');
  if ((pw.match(/[^A-Za-z0-9]/g) || []).length < 2) errors.push('At least 2 special characters (e.g. !@#$%^&*)');
  return errors;
}
...
async function main() {
  ...
  console.log('    • At least 2 digits');
  console.log('    • At least 2 special characters\n');
